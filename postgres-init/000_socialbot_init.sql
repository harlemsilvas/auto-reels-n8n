...
CREATE TABLE IF NOT EXISTS ai_provider_credentials (
    ...
    CONSTRAINT ai_provider_credentials_label_check
      CHECK (LENGTH(BTRIM(label)) BETWEEN 1 AND 120),
    CONSTRAINT ai_provider_credentials_task_check
      CHECK (task IN ('media_templates_text', 'inbox_reply', 'content_review', 'general_test')),
    CONSTRAINT ai_provider_credentials_model_check
      CHECK (LENGTH(BTRIM(model)) BETWEEN 1 AND 120),
    CONSTRAINT ai_provider_credentials_status_check
      CHECK (status IN ('active', 'limited', 'expired', 'disabled')),
    CONSTRAINT ai_provider_credentials_limits_check CHECK (
      priority >= 0
...
CREATE TABLE IF NOT EXISTS ai_provider_usage_events (
    ...
    details JSONB NOT NULL DEFAULT '{}'::jsonb,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT ai_provider_usage_events_event_type_check
      CHECK (event_type IN ('selected', 'request_success', 'request_error', 'rate_limited', 'expired', 'disabled'))
);
